...
namespace lab4
{
    class Program
    {
        const string HelpDesk = "\t/share *name of share* - returns extended information about the share\n" +
        ...
        private static void Bot_OnMessage(object sender, Telegram.Bot.Args.MessageEventArgs e)
        {
            ...
            string userMess = e.Message.Text;
            var userMessWord = userMess.Split(new string[] { " " }, StringSplitOptions.RemoveEmptyEntries);
            var count = userMessWord.Length;


            if ((count == 1 && userMessWord[0][0] != '/') || (userMessWord[0].ToString() == "/share"))
            {
                Share_Info(sender, e);
                return;
            }
            ...
            if (userMessWord[0].ToString() == "/list")
            {
            ...
            if (userMessWord[0].ToString() == "/add")
            {
            ...
            if (userMessWord[0].ToString() == "/help")
            {
                bot = sender as TelegramBotClient;
            ...
            if (userMessWord[0].ToString() == "/delete")
            {
                if (userMessWord[1].ToString() == "&all")
                {
        ...
        private static void Share_Info(object sender, Telegram.Bot.Args.MessageEventArgs e, bool shortmess = false, string ShareCode = null, int cost = 0)
        {
            var bot = sender as TelegramBotClient;

            var url = "https://finance.yahoo.com/quote/";

            var userMessWord = e.Message.Text.Split(new string[] { " " }, StringSplitOptions.RemoveEmptyEntries);
            var count = userMessWord.Length;
            ...
            var ShareVal = SharePlat[SharePlat.Length - 3].ToString() +
                           SharePlat[SharePlat.Length - 2].ToString() + SharePlat[SharePlat.Length - 1].ToString();
        ...
        {
            for (int i = 1; i < ShareCodes.Length; i += 1)
            {
                if(!Add_Share(ShareCodes[i])) return false;
            }
            return true;
        }
        ...
        private static bool Del_Share(string ShareCode)
        {
            ...
            {
                if (share.Name == ShareCode) AddedShares.Remove(share);
            }
            SharesQuant -= 1;
            return true;
        }
        ...
        private static bool Del_Several_Shares(string[] ShareCodes)
        {
            for (int i = 1; i < ShareCodes.Length; i += 1)
            {
                if (!Del_Share(ShareCodes[i])) return false;
            }
            AddedShares = AddedShares.ToList();
            return true;
        }

        private static bool List_Share(object sender, Telegram.Bot.Args.MessageEventArgs e)
        {
            foreach (var share in AddedShares)
            {
                Share_Info(sender, e, true, share.Name);
            }
